...
import { Bitcoin, CircleDot, Rocket } from "lucide-react";
...
export default function App() {
  ...
  // Auto-run the default simulation once the model is ready, so the dashboard
  // shows skeletons → real results on first load instead of an empty panel.
  useEffect(() => {
    ...
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
      ...
      <header className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold tracking-tight">Bitcoin Sentiment Trading Simulation</h1>
      ...
      <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
        ...
        <main className="space-y-4">
          {sim.isPending || health.isLoading || (modelLoaded && !result && !sim.isError) ? (
            ...
            <motion.div
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
